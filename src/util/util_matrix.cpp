...
namespace dxvk {
  ...
  bool Matrix4::operator==(const Matrix4& m2) const {
    ...
    __m128i r0 = _mm_castps_si128(_mm_cmpeq_ps(_mm_load_ps(&data[0].x), _mm_load_ps(&m2.data[0].x)));
    __m128i r1 = _mm_castps_si128(_mm_cmpeq_ps(_mm_load_ps(&data[1].x), _mm_load_ps(&m2.data[1].x)));
    __m128i r2 = _mm_castps_si128(_mm_cmpeq_ps(_mm_load_ps(&data[2].x), _mm_load_ps(&m2.data[2].x)));
    __m128i r3 = _mm_castps_si128(_mm_cmpeq_ps(_mm_load_ps(&data[3].x), _mm_load_ps(&m2.data[3].x)));
    ...
    for (uint32_t i = 0; i < 4; i++) {
      __m128 r1 = _mm_load_ps(&data[i].x);
      __m128 r2 = _mm_load_ps(&m2.data[i].x);
  ...
  Matrix4 Matrix4::operator+(const Matrix4& other) const {
    Matrix4 mat;
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&mat.data[i].x, _mm_add_ps(_mm_load_ps(&data[i].x), _mm_load_ps(&other.data[i].x)));
    }
    return mat;
  }
  ...
  Matrix4 Matrix4::operator-(const Matrix4& other) const {
    Matrix4 mat;
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&mat.data[i].x, _mm_sub_ps(_mm_load_ps(&data[i].x), _mm_load_ps(&other.data[i].x)));
    }
    return mat;
  }
  ...
  Matrix4 Matrix4::operator*(const Matrix4& m2) const {
    Matrix4 result;
    __m128 rowB0 = _mm_load_ps(&m2.data[0].x);
    __m128 rowB1 = _mm_load_ps(&m2.data[1].x);
    __m128 rowB2 = _mm_load_ps(&m2.data[2].x);
    __m128 rowB3 = _mm_load_ps(&m2.data[3].x);

    for (uint32_t i = 0; i < 4; i++) {
      __m128 rowA = _mm_load_ps(&data[i].x);
      ...
      _mm_store_ps(&result.data[i].x, _mm_add_ps(sum01, sum23));
    }
    return result;
  }
  ...
  Vector4 Matrix4::operator*(const Vector4& v) const {
    ...
    __m128 row0 = _mm_load_ps(&data[0].x);
    __m128 row1 = _mm_load_ps(&data[1].x);
    __m128 row2 = _mm_load_ps(&data[2].x);
    __m128 row3 = _mm_load_ps(&data[3].x);
    ...
    __m128 sum01 = _mm_add_ps(v0, v1);
    __m128 sum23 = _mm_add_ps(v2, v3);
    _mm_store_ps(&result.x, _mm_add_ps(sum01, sum23));
  #endif
  ...
  Matrix4 Matrix4::operator*(float scalar) const {
    ...
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&result.data[i].x, _mm_mul_ps(_mm_load_ps(&data[i].x), s));
    }
    return result;
  }
  ...
  Matrix4 hadamardProduct(const Matrix4& a, const Matrix4& b) {
    Matrix4 result;
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&result.data[i].x, _mm_mul_ps(_mm_load_ps(&a.data[i].x), _mm_load_ps(&b.data[i].x)));
    }
    return result;
  }
  ...
  Matrix4& Matrix4::operator+=(const Matrix4& other) {
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&data[i].x, _mm_add_ps(_mm_load_ps(&data[i].x), _mm_load_ps(&other.data[i].x)));
    }
    return *this;
  }
  ...
  Matrix4& Matrix4::operator-=(const Matrix4& other) {
    for (uint32_t i = 0; i < 4; i++) {
      _mm_store_ps(&data[i].x, _mm_sub_ps(_mm_load_ps(&data[i].x), _mm_load_ps(&other.data[i].x)));
    }
    return *this;
  }
  ...
  Matrix4 transpose(const Matrix4& m) {
    Matrix4 result;
    __m128 r0 = _mm_load_ps(&m.data[0].x);
    __m128 r1 = _mm_load_ps(&m.data[1].x);
    __m128 r2 = _mm_load_ps(&m.data[2].x);
    __m128 r3 = _mm_load_ps(&m.data[3].x);
    ...
    _mm_store_ps(&result.data[0].x, _mm_movelh_ps(t0, t2));
    _mm_store_ps(&result.data[1].x, _mm_movehl_ps(t2, t0));
    _mm_store_ps(&result.data[2].x, _mm_movelh_ps(t1, t3));
    _mm_store_ps(&result.data[3].x, _mm_movehl_ps(t3, t1));
