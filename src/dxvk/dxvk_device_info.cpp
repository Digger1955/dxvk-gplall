...
/* Probably not needed since already in dxvk_adapter.cpp
...
*/
    ...
    if (m_featuresSupported.extPageableDeviceLocalMemory.pageableDeviceLocalMemory)
      m_featuresSupported.amdMemoryOverallocationBehaviour = VK_FALSE;
    ...
    if (m_properties.vk12.driverID == VK_DRIVER_ID_NVIDIA_PROPRIETARY && m_properties.driverVersion >= Version(535u, 0u, 0u)) {
...
*/
      ...
      /* Controlling whether explicit overallocation beyond the device memory heap sizes is allowed or not */
      ENABLE_EXT(amdMemoryOverallocationBehaviour, false),

      /* Efficient read access to the fragment mask in compressed multisampled color surfaces on AMD */
...
*/
      ...
      /* DescriptorPoolOverallocation, improves performance on NV */
