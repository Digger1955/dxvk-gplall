...
void main() {
  ...
  // - 3: top row (00 + 10)
  // - 5: left column (00 + 01)
  // Default: fallback to full-4 fetch inner loop (generic, correct) which also handles mask==15.
  switch (mask) {
    case 1u: { // 00 (top-left)
      [[unroll]]
      for (uint s = 0u; s < c_src_samples; ++s) {
    ...
    default: { // fallback: full 4-fetch generic loop (covers mask==15 and others)
      [[unroll]]
      for (uint s = 0u; s < c_src_samples; ++s) {
        int sample_idx = int(s);
        vec4 c00 = texelFetch(s_image_ms, ivec3(coord_00, gl_Layer), sample_idx);
